...
def agendar_lembrete(db: Session, agendamento) -> None:
    ...
    db.add(Notificacao(agendamento_id=agendamento.id, canal=os.getenv("NOTIFICACAO_CANAL", "sms"), enviar_em=max(enviar_em, datetime.now()), status="pendente", tentativas=0))
    db.commit()


def processar_pendentes(db: Session) -> int:
    pendentes = db.query(Notificacao).filter(Notificacao.status == "pendente", Notificacao.enviar_em <= datetime.now()).all()
    enviados = 0
